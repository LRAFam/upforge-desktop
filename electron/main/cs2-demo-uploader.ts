...
export interface CS2DemoUploadOptions {
  demoPath: string
  steamId?: string | null
  onProgress: (pct: number) => void
}

export interface CS2DemoUploadResult {
  jobId: string
}

export class CS2DemoUploader {
  constructor(private auth: AuthManager) {}

  /**
   * Upload a CS2 demo using the three-step flow:
   *   1. POST /api/cs2/demo/presign  → get { job_id, upload_url }
   *   2. PUT <s3-url>                → stream file directly to S3
   *   3. POST /api/cs2/demo/confirm  → confirm upload and start analysis
   */
  async upload(opts: CS2DemoUploadOptions): Promise<CS2DemoUploadResult> {
    ...
    const presignResult = await this._apiPost(`${apiUrl}/api/cs2/demo/presign`, presignBody, token)
  ...
  /** Poll the analysis status for a given job. */
  async pollStatus(jobId: string): Promise<{ status: string; error?: string | null }> {
    const apiUrl = process.env['VITE_API_URL'] || 'https://api.upforge.gg'
    const token = this.auth.getToken()
    if (!token) throw new Error('Not authenticated')

    return new Promise((resolve, reject) => {
      const url = new URL(`${apiUrl}/api/cs2/demo/${jobId}/status`)
