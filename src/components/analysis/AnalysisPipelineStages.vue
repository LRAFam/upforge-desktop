...
<template>
  <div class="grid grid-cols-4 gap-1.5">
    <div
      ...
      :key="stage.id"
      class="rounded-xl border px-2 py-2 text-left transition-colors"
      :class="{
    ...
    >
      <div class="flex items-center gap-1.5">
        ...
        <p
          class="text-[10px] font-bold uppercase tracking-[0.14em] truncate"
          :class="stageState(index) === 'pending' ? 'text-gray-600' : 'text-gray-200'"
        >
          {{ stage.label }}
        </p>
      </div>
      <p class="mt-1 text-[9px] leading-snug text-gray-500 line-clamp-2">{{ stage.detail }}</p>
    </div>
  </div>
</template>
