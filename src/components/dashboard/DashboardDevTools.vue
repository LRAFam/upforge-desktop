  ...
  <div v-if="isDev || (platform && platform !== 'win32')" class="border border-dashed border-yellow-500/20 rounded-xl overflow-hidden flex-shrink-0">
    ...
    <div v-if="devOpen" class="px-3 pb-3 space-y-2">
      <div class="flex gap-2">
        ...
        <button class="px-2 py-1.5 rounded-lg text-xs font-medium bg-sky-500/[0.08] text-sky-400/80 hover:bg-sky-500/[0.14] transition-colors border border-sky-500/15" @click="$router.push({ path: '/onboarding', query: { preview: '1' } })">Onboarding</button>
        <button class="px-2 py-1.5 rounded-lg text-xs font-medium bg-violet-500/[0.08] text-violet-300/80 hover:bg-violet-500/[0.14] transition-colors border border-violet-500/15" @click="$router.push({ path: '/onboarding', query: { preview: '1' } })">Setup wizard</button>
