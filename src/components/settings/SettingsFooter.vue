  ...
  <div class="flex-shrink-0 space-y-2 border-t border-white/[0.07] bg-[#161616] px-3 pt-2 pb-3">
    <div class="flex items-center justify-between px-0.5">
      ...
      <div class="flex items-center gap-3">
        ...
        <button type="button" class="text-xs text-gray-600 transition-colors hover:text-gray-400" @click="openHelp">Get help</button>
        <button type="button" class="text-xs text-gray-600 transition-colors hover:text-gray-400" @click="openSite">upforge.gg</button>
      </div>
    </div>
    <div v-if="devModeActive" class="flex items-center justify-between rounded-lg border border-amber-500/20 bg-amber-500/6 px-3 py-2">
      ...
      <button type="button" class="text-xs text-amber-600 transition-colors hover:text-amber-400" @click="disableDevMode">Disable</button>
    </div>
  </div>
</template>
