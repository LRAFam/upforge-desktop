...
const WEAPON_CDN_UUIDS: Record<string, string> = {
...
/** Returns the display-icon CDN URL for a Valorant weapon name. */
export function getWeaponImage(weaponName: string | null | undefined): string {
  if (!weaponName) return ''
  const key = weaponName.toLowerCase().replace(/[/\s-]/g, '')
  const uuid = WEAPON_CDN_UUIDS[key]
